...
def epsilon_greedy_action(q, state, epsilon):
    ...
    prob_vect = [epsilon/m for k in range(m)]
    prob_vect[argmax] = epsilon/m + 1 - epsilon
...
class Sarsa(RLAlgorithm):
    ...
    def learn(self, state, action, reward, next_state, next_action):
        # [DONE] Todo: implement
        aux = reward + self.gamma * self.q[next_state][next_action] - self.q[state][action]
        self.q[state][action] = self.q[state][action] + self.alpha * aux
...
class QLearning(RLAlgorithm):
    ...
    def learn(self, state, action, reward, next_state, next_action):
        # [DONE] Todo: implement
        m = self.q.shape[1]
        maxQ = np.max([self.q[next_state][a] for a in range(m)])
        aux = reward + self.gamma * maxQ - self.q[state][action]
        self.q[state][action] = self.q[state][action] + self.alpha * aux
